...
__global__ void Marlin(
  ...
  bool in_part2 = false;
  printf("[GPU %d] Marlin kernel launched with prob_m=%d, prob_n=%d, prob_k=%d, parallel=%d, k_tiles=%d, n_tiles=%d, thread_k_blocks=%d, thread_n_blocks=%d, moe_block_size=%d, gridDim.x=%d\n",
         blockIdx.x, prob_m, prob_n, prob_k, parallel, k_tiles, n_tiles,thread_k_blocks, thread_n_blocks, moe_block_size, gridDim.x);
