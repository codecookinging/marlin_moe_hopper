...
for arch in sys.argv[1].split(","):
    ...
    if arch == 75:
        SUPPORT_SM75 = True
    # H100 (SM90) has ~228 KB opt-in shared memory vs ~164 KB on A100, which
    # allows a 5-stage async pipeline.  The generated SM90 kernels also use
    # legal L2 prefetch-size hints for reusable cp.async weight/scale loads
    # (see marlin_hopper.cuh).
    if arch >= 90:
        SUPPORT_SM90 = True
    ARCHS.append(arch)
...
THREAD_CONFIGS = [(128, 128, 256), (64, 256, 256), (64, 128, 128), (128, 64, 128), (64, 256, 128)]
...
def remove_old_kernels():
    ...
    # Also remove previously generated sm90 kernel files
    for filename in glob.glob(os.path.dirname(__file__) + "/sm90_kernel_*.cu"):
        subprocess.call(["rm", "-f", filename])
...
def generate_new_kernels():
    result_dict = {}
    sm_75_result_dict = {}
    sm_90_result_dict = {}  # stages=5 kernels for H100 / SM90+
    ...
    for quant_config in QUANT_CONFIGS:
        ...
        for a_type, c_type in itertools.product(a_types, c_types):
            ...
            if (a_type, b_type, c_type) not in result_dict:
                ...
                if SUPPORT_SM90:
                    sm_90_result_dict[(a_type, b_type, c_type)] = []
            ...
            for group_blocks, m_blocks, thread_configs in itertools.product(
                ...
                # SM90 (H100): 5-stage pipeline — larger shared memory allows
                # one extra prefetch stage to hide global memory latency.
    ...
    for result_dict_tmp in [result_dict, sm_75_result_dict, sm_90_result_dict]:
        for (a_type, b_type, c_type), config_list in result_dict_tmp.items():
            ...
            elif result_dict_tmp is sm_90_result_dict:
                filename = f"sm90_kernel_{a_type[1:]}_{b_type[1:]}_{c_type[1:]}.cu"
            else:
